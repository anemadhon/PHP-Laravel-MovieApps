...
@section('content')
    <div class="actor-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <div class="flex-none">
                <img src="{{ $actor['profile_path'] }} " alt="profile-path-{{ $actor['name'] }}" class="w-64 lg:w-96">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ $actor['birthday_details'] }}</span>
                </div>
                <p class="text-gray-300 mt-8">{{ $actor['biography'] }}</p>
                <h4 class="font-semibold mt-9">Known For</h4>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    @foreach ($knownForDesc as $desc)
                        <div class="mt-4">
                            <a href="{{ route('movies.show', $desc['id']) }}"><img src="{{ $desc['poster_path'] }}" alt="poster-known-for-{{ $desc['title'] }}"></a>
                            <a href="{{ route('movies.show', $desc['id']) }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $desc['title'] }}</a>
                        </div>
                    @endforeach
                </div>
            </div>
